...
struct Sales_data {
  public:
    Sales_data(): Sales_data("",0,0) {std::cout << "default const initi" << std::endl; }
    Sales_data(std::string s): Sales_data(s,0,0) {std::cout << "bookNo-para const initi" << std::endl; }
    //Sales_data(std::istream& is = std::cin) Sales_data(){is >> (*this).bookNo;  }
    // constructer only with bookNo -2
    //Sales_data(const std::string& s): bookNo(s) { }
    // normal constructure -3
    Sales_data(const std::string& s, unsigned n, double p): bookNo(s), units_sold(n), revenue(p*n) { std::cout << "all three param-input const initi" << std::endl;}
    // istream constructer -4
    //constructer definition for -4
    Sales_data(std::istream &is): Sales_data() {
      Sales_data::read(is, *this);
      std::cout << "const init - istream input" << std::endl;
    }
...
int main() {
  ...
  Sales_data lasttwo("9-999-99999-9", 20, 50);
  print(std::cout, lasttwo) << "<-- member values for lasttwo initializer" << std::endl;
}
/*
int main(){
  Sales_data test;
